...
def generate(odict_entrada):
    ...
    for conta in odict_entrada['segmento_a_contas']:
        ...
        """
        seu_numero_complemento = str(datetime.now().strftime(
            "%y")) + str(datetime.now().timetuple().tm_yday) + str(datetime.now().strftime("%H%M"))
        if('credito_seu_numero' in conta.keys()):
            ...
            odic_sega['credito_seu_numero'] = conta['cpf'] + \
                seu_numero_complemento  # G064
    ...
    # 1 para header do lote e 1 para trailer do lote
    ...
    odic_trailer_arquivo['quantidade_registros'] = 1 + \
        1 + 1 + 1 + sequencial_registro
